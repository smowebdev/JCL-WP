...
>
    <div class="flex items-center justify-between gap-s">
        ...
        >
            @if ($logo)
                <img
                    class="h-6 w-auto md:h-6.5 lg:h-8"
                    src="{{ $logo['url'] }}"
        ...
        <button
            type="button"
            class="menu-toggle block cursor-pointer md:hidden"
                ...
                @foreach ($socials as $social)
                    ...
                    @elseif ($type === 'qr' && !empty($qr['url']))
                        <div class="relative group js-wechat cursor-pointer">
                            <button
                                type="button"
                                class="cursor-pointer js-wechat-trigger"
                            >
                                @if (!empty($icon['url']))
                                    <img
                                        class="h-6 w-6 object-contain"
                            ...
                            <div
                                class="absolute left-1/2 bottom-full z-50 pb-2 hidden -translate-x-1/2 md:group-hover:block js-wechat-qr">
                                <img
                                    class="w-24 h-24 max-w-[unset] object-contain bg-white p-2 shadow-lg"
                                    src="{{ $qr['url'] }}"
                                    alt="{{ $qr['alt'] }}"
                                >
                            </div>
